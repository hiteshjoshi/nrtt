module.exports = (function() {
  ...
  var Narad = function (options) {
    this.options = options;
    this.options.domain = this.options.domain || 'api.nrtt.in';
  ...
  Narad.prototype.trigger = function (channel, event, message, callback) {
    ...
    var hash = Narad.modules.crypto.createHash('md5').update(new Buffer(requestBody).toString('binary')).digest('hex');
    ...
    var signature = Narad.modules.crypto.createHmac('sha256', this.options.secret).update(signData).digest('hex');
